(function(){
  ...
  (function(){
    ...
    // Service pre-fill from ?service= query param (set by CTAs across the site)
    const svcParam = params.get('service');
    if(svcParam){
      const match = Array.from(serviceSelect.options).find(o => o.value.toLowerCase() === svcParam.toLowerCase());
      if(match) serviceSelect.value = match.value;
    }

    // UTM + landing page + referrer capture
    document.getElementById('cfUtmSource').value = params.get('utm_source') || '';
    document.getElementById('cfUtmMedium').value = params.get('utm_medium') || '';
    document.getElementById('cfUtmCampaign').value = params.get('utm_campaign') || '';
    document.getElementById('cfLandingPage').value = location.pathname + location.search;
    document.getElementById('cfReferrer').value = document.referrer || '';

    function showError(field){ const el = form.querySelector(`[data-error-for="${field}"]`); if(el) el.classList.add('show'); }
    function hideError(field){ const el = form.querySelector(`[data-error-for="${field}"]`); if(el) el.classList.remove('show'); }
    ['name','email','service','details'].forEach(f=>{
      const el = form.querySelector(`[name="${f}"]`);
      if(el) el.addEventListener('input', ()=> hideError(f));
    });
    ...
    function validate(){
      ...
      const details = document.getElementById('cfDetails').value.trim();
      if(!name){ showError('name'); ok = false; }
      if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){ showError('email'); ok = false; }
      if(!service){ showError('service'); ok = false; }
      if(details.length < 3){ showError('details'); ok = false; }
      return ok;
    }
    ...
    form.addEventListener('submit', async (e)=>{
      ...
      try {
        ...
        window.edgewebLeads.push(data);
        if(typeof trackEvent === 'function') trackEvent('contact_form_submit', { service: data.service, budget: data.budget });

        document.getElementById('formWrap').querySelector('.eyebrow').style.display='none';
        form.style.display = 'none';
        document.getElementById('formSuccess').classList.add('show');
        status.textContent = 'Sent successfully';
      } catch (error) {
        console.error('Contact form email submission failed:', error);
        status.textContent = 'Something went wrong. Please email us directly at info@edgeweb.co.';
        status.classList.add('show');
